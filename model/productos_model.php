...
class mostrar{
    ...
    public function mostrarProductos() {
      ...
      $query = "SELECT nombre_producto,id_producto,precio,foto,stock from producto where producto.status=1";
    ...
    public function mostrarTodosProductos() {
        ...
        //select nombre_producto,precio,foto,stock from producto where producto.status=1;
      $query = "SELECT nombre_producto,id_producto,precio,foto,stock,status from producto";
        $stmt = $this->db->prepare($query);
        $stmt->execute();
        $cursos = array(); // Inicializa un array para almacenar los cursos
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
